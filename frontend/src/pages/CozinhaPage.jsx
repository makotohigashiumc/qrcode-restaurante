...
function CardPedido({ pedido, onAtualizar }) {
  ...
  return (
    ...
    ><div className="flex items-start justify-between gap-2"><div><p className="font-bold text-gray-900 text-base">Mesa #{pedido.mesa_numero}</p>
          {pedido.nome_cliente && <p className="text-xs text-gray-500"> {pedido.nome_cliente}</p>}
          <p className="text-xs text-gray-400">{formatTime(pedido.criado_em)}</p></div><span className={`text-xs font-semibold px-2 py-1 rounded-full whitespace-nowrap ${cfg.cor}`}>{cfg.label}</span></div><ul className="space-y-1">
        {(pedido.itens || []).map((it, i) => (
          <li key={i} className="flex justify-between text-sm"><span className="text-gray-700">{it.item_nome || it.nome}</span><span className="font-semibold text-gray-900">× {it.quantidade}</span></li>
        ))}
      </ul>
...
export default function CozinhaPage() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center"><div className="animate-spin rounded-full h-10 w-10 border-b-2 border-beni-mid" /></div>
    )
  }

  if (!token) {
    return (
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center p-6"><div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 max-w-md w-full text-center"><h1 className="text-xl font-bold">‍ Painel da Cozinha</h1><p className="text-gray-300 text-sm mt-2">
            Para ver os pedidos, você precisa estar logado no painel admin.
          </p><a
            href="/admin/login"
            className="inline-block mt-4 bg-beni-mid hover:opacity-80 text-white px-5 py-2.5 rounded-xl font-semibold"
          >
            Ir para o login
          </a></div></div>
    )
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white">
      {/* Header */}
      <header className="bg-gray-800 px-6 py-4 flex items-center justify-between border-b border-gray-700"><div><h1 className="text-xl font-bold">‍ Painel da Cozinha</h1><p className="text-gray-400 text-xs">Atualização em tempo real via WebSocket</p></div><div className="flex gap-3">
          {[
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors
                ${filtro === f.key ? 'bg-beni-mid text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'}`}
            >
              {f.label}
            </button>
          ))}
          <button onClick={() => refetch()} className="px-3 py-1.5 bg-gray-700 rounded-lg text-sm hover:bg-gray-600"></button></div></header>

      {/* Status bar */}
      <div className="bg-gray-800 px-6 py-3 flex gap-6 text-sm border-b border-gray-700"><span className="text-blue-400"> Recebidos: <strong>{counts.recebido}</strong></span><span className="text-yellow-400"> Em Preparo: <strong>{counts.em_preparo}</strong></span><span className="text-green-400"> Prontos: <strong>{counts.pronto}</strong></span></div>

      {isError && (
        <div className="px-6 pt-6"><div className="bg-gray-800 border border-gray-700 rounded-2xl p-5"><p className="font-semibold">
              {error?.response?.status === 503
                ? 'Banco indisponível'
                : !error?.response
                  ? 'Servidor offline'
                  : 'Erro ao carregar pedidos'}
            </p><p className="text-sm text-gray-300 mt-1">
              {error?.response?.status === 503
              ...
              onClick={() => refetch()}
              className="mt-3 bg-beni-mid hover:opacity-80 text-white px-4 py-2 rounded-xl text-sm font-semibold"
            >
               Tentar novamente
            </button></div></div>
      )}
      ...
      <div className="p-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {pedidosFiltrados.length === 0 ? (
          <div className="col-span-full text-center py-20 text-gray-500"><div className="text-5xl mb-3"></div><p>Nenhum pedido no momento</p></div>
        ) : (
